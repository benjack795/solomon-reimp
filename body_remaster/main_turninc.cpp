...
#define ANKLESWITCH 1

//bird body
#define SPHERERAD 20 //(BODLENGTH/2) + UPPERLENGTH + LOWERLENGTH
#define SPHERE2RAD 0.015
#define SPHERE2RADUP 2
...
#define DRAWSPHERE 0
#define TORQUECOSTY 30*(1/TSTEP)*TORQUEREMOVER
...
struct Controller
{
	...
	//normalise inputs 
	//input without excluded bits (15-18)
	reducedinput.clear();
	...
	//printf(" \n");
	//base largest and smallest on that instead
...
double segs(double x, double y, double z, double x2, double y2, double z2, double segnum, dGeomID g){
	...
	double rotmatT[3][3] = {  
   	{bodyrot[0], bodyrot[4], bodyrot[8]} ,   
   	{bodyrot[1], bodyrot[5], bodyrot[9]} ,   
   	{bodyrot[2], bodyrot[6], bodyrot[10]}   
	};
	//multiply it by the transpose of the rotation matrix
	double finalmat[3][3] = {
	{(vector1[0]*rotmatT[0][0] + vector1[0]*rotmatT[1][0] + vector1[0]*rotmatT[2][0]),
	(vector1[0]*rotmatT[0][1] + vector1[0]*rotmatT[1][1] + vector1[0]*rotmatT[2][1]),
	(vector1[0]*rotmatT[0][2] + vector1[0]*rotmatT[1][2] + vector1[0]*rotmatT[2][2])},
	{(vector1[1]*rotmatT[0][0] + vector1[1]*rotmatT[1][0] + vector1[1]*rotmatT[2][0]),
	(vector1[1]*rotmatT[0][1] + vector1[1]*rotmatT[1][1] + vector1[1]*rotmatT[2][1]),
	(vector1[1]*rotmatT[0][2] + vector1[1]*rotmatT[1][2] + vector1[1]*rotmatT[2][2])},
	{(vector1[2]*rotmatT[0][0] + vector1[2]*rotmatT[1][0] + vector1[2]*rotmatT[2][0]),
	(vector1[2]*rotmatT[0][1] + vector1[2]*rotmatT[1][1] + vector1[2]*rotmatT[2][1]),
	(vector1[2]*rotmatT[0][2] + vector1[2]*rotmatT[1][2] + vector1[2]*rotmatT[2][2])}
	};
	//atan2 that
	double segangle = atan2(finalmat[0][0],finalmat[1][0]);
	//place angle within appropriate segment with ceil
	if (segangle < 0) { 
		segangle += 2 * PI; 
	}
	segangle = segangle / DEG2RAD;
	double segin = ceil(segangle / (360/segnum));
	//printf("seg %f \n", segin);
	return segin;
}

double segshard(double x, double y, double z, double x2, double y2, double z2, double segnum, dGeomID g){
	//make two vectors
	double vector1[2];
	double vector2[2];
	...
	vector2[0] = x2-x;
	vector2[1] = y2-y;
	//find angle between two vectors with trig
...
struct Walker {
    int i;
    std::vector<double> geno, hippo, stepwid, reacts;
...
-(UPPERLENGTH/2)*sin(r_th_ang), 
-(HIPLENGTH/2),
startz-(HIPRADIUS/2)-((UPPERLENGTH/2)*cos(r_th_ang)));
        //set thigh rotation
        dMatrix3 rot_ptr;
...
-(UPPERLENGTH/2)*sin(l_th_ang), 
(HIPLENGTH/2),
startz-(HIPRADIUS/2)-((UPPERLENGTH/2)*cos(l_th_ang)));
        //set thigh rotation
        dMatrix3 rot_ptr2;
	...
	kneepos_ptr[2] = hippos_ptr[2]-(UPPERLENGTH)*cos(r_th_ang);
        ...
        r_sh_ang = -45*DEG2RAD;
        dBodySetPosition (body[4], 
kneepos_ptr[0] - (LOWERLENGTH/2)*sin(r_sh_ang) + 0.0011*cos(r_sh_ang),
kneepos_ptr[1] +0.0140,
kneepos_ptr[2]- (LOWERLENGTH/2)*cos(r_sh_ang) + 0.0011*sin(r_sh_ang));
        dMatrix3 rot_ptr3;
	...
	kneepos_ptr2[2] = hippos_ptr2[2]-(UPPERLENGTH)*cos(l_th_ang);
        ...
        l_sh_ang = -60*DEG2RAD;
        dBodySetPosition (body[5], 
kneepos_ptr2[0] - (LOWERLENGTH/2)*sin(l_sh_ang) + 0.0011*cos(l_sh_ang),
kneepos_ptr2[1]-0.0140,
kneepos_ptr2[2]- (LOWERLENGTH/2)*cos(l_sh_ang) + 0.0011*sin(l_sh_ang));
        dMatrix3 rot_ptr4;
	...
	ank_ptr[2] = kneepos_ptr[2]-(LOWERLENGTH)*cos(r_sh_ang);
        ...
        r_an_ang = 0*DEG2RAD;
        dBodySetPosition (body[6], 
ank_ptr[0]+(FOOTHEIGHT/2)*sin(r_an_ang),
ank_ptr[1],
ank_ptr[2] -(FOOTHEIGHT/2)*cos(r_an_ang));
        dMatrix3 rot_ptr5;
	...
	ank_ptr2[2] = kneepos_ptr2[2]-(LOWERLENGTH)*cos(l_sh_ang);
        ...
        l_an_ang = -10*DEG2RAD;
        dBodySetPosition (body[7], 
ank_ptr2[0] +(FOOTHEIGHT/2)*sin(l_an_ang),
ank_ptr2[1],
ank_ptr2[2] -(FOOTHEIGHT/2)*cos(l_an_ang));
        dMatrix3 rot_ptr6;
	...
	if(ANKLESWITCH){
		...
		dJointSetUniversalParam (joint[5],dParamHiStop2,0*DEG2RAD);
        	dJointSetUniversalParam (joint[5],dParamLoStop2,0*DEG2RAD);
	}
	...
	if(ANKLESWITCH){
		...
		dJointSetUniversalParam (joint[6],dParamHiStop2,0*DEG2RAD);
        	dJointSetUniversalParam (joint[6],dParamLoStop2,0*DEG2RAD);
	}
    ...
    void draw()
    {
	  ...
	  //dJointGetHingeAnchor(joint[6],jointpos6ex);
	  //printf("%f %f %f \n",jointpos6ex[0],jointpos6ex[1],jointpos6ex[2]);
...
//          }
	...
	//phi x theta y faiz z
	if(R[8] != 1 && R[8] != -1){
		theta = -asin(R[8]);
		phi = atan2((R[9]/cos(theta)),(R[10]/cos(theta)));
		faiz = atan2((R[4]/cos(theta)),(R[0]/cos(theta)));
	}else{
		faiz = 0;
		if(R[8] == -1){
			theta = PI/2;
			phi = faiz + atan2(R[1],R[2]);
		}
		else{
			theta = -PI/2;
			phi = -faiz + atan2(-R[1],-R[2]);
    ...
    std::vector<double> nueulers(const dReal* R){
	dReal theta, phi, faiz;
	...
	dReal ih, yawangle, rollangle;
	    ...
	    //3d inputs
	    std::vector<double> bodangle3d = nueulers(dGeomGetRotation(box[1]));
	    ...
	    //24 upper roll angle (x)
           ...
           //26 - SENSOR
            ...
            filconts[i].input.at(26) = 	segshard(double(hippos[0]),double(hippos[1]),double(hippos[2]),double(goalpoint[0]),double(goalpoint[1]),double(goalpoint[2]),8, box[0]);
...
//                //5 swing ankle
	...
	dReal fitinc = (hidist-dist)/10000;
        totalfit += (fitinc + (hidist/10000)*nummoves); 

	
	//moving point
	double maxmag = MAXMAG;
	double wlnth = WLNTH;
	double finalmag = maxmag * min((double)1,((0.5*curgen)/GENS));
	if (dist < 1){
		goalpoint[0] += wlnth/4;
		switch(pointpos){
			case 1:
				goalpoint[1] += finalmag;
				pointpos = 2;
			break;
			case 2:
				goalpoint[1] -= finalmag;
        ...
        if(HIPPOCHECK){
		const dReal* hippopos = dBodyGetPosition(body[0]);
                if(iswinner && (age % 50 == 0)){
                	hippo.push_back(hippopos[2]);		
                }
	}

        if(STEPWID){
		const dReal* lfopos = dBodyGetPosition(body[6]);
		const dReal* rfopos = dBodyGetPosition(body[7]);
		dReal distx = lfopos[0] - rfopos[0]; 
	        dReal disty = lfopos[1] - rfopos[1];
		dReal dist = sqrt(pow(distx,2) + pow(disty,2));
                if(iswinner && (age % 50 == 0)){
                	stepwid.push_back(dist);		
                }
	}

        if(RECACTS){
                if(iswinner && (age % 50 == 0)){
			reacts.push_back(bodangle/DEG2RAD);
			reacts.push_back(il/DEG2RAD);
			reacts.push_back(t_st/DEG2RAD);
			reacts.push_back(k_st/DEG2RAD);
			reacts.push_back(a_st/DEG2RAD);
			reacts.push_back(k_sw/DEG2RAD);
			reacts.push_back(a_sw/DEG2RAD);		
                }
	}
...
static std::vector<double> fitnesses, bigfitnesses, moveses, mutations, longdiff, massdiff, torquesleft;
...
static void command (int cmd)
    {
      switch (cmd) {
          case 'w': case 'W':
          dBodyAddForce(winner->body[1], 0,0,10000);
            break;
          case 's': case 'S':
          break;
          case 'a': case 'A':
...
void writeFilesSetSeed(){
    ...
    fitfile << "generation FIL =" << ISFIL << ",fittest," << "torque left," << "moves \n";
...
void write_hippo(){
    ...
    std::ofstream hippofile;
    std::string filenameh = fileseed;
    filenameh += "_hipstable";
    filenameh += ".csv";
    hippofile.open(filenameh);
    int hipposize = winner->hippo.size();
    for(int i = 0; i < hipposize; i++){
        char buf[50];
        snprintf(buf, 50, "%f", winner->hippo.at(i));
        hippofile << buf << "\n";
    }
    hippofile.close();
}
...
void write_stepwid(){
    ...
    std::ofstream widfile;
    std::string filenames = fileseed;
    filenames += "_stepwidth";
    filenames += ".csv";
    widfile.open(filenames);
    int stepsize = winner->stepwid.size();
    for(int i = 0; i < stepsize; i++){
        char buf[50];
        snprintf(buf, 50, "%f", winner->stepwid.at(i));
        widfile << buf << "\n";
    }
    widfile.close();
}
...
void write_reacts(){
    ...
    std::string filenamea = fileseed;
    filenamea += "_activation.csv";
    actfile.open(filenamea);
    int actssize = winner->reacts.size();
    for(int i = 0; i < (actssize/7); i++){
        for(int j = 0; j < 7; j++){
            char buf2[50];
            snprintf(buf2, 50, "%f", winner->reacts.at((i*7)+j));
            actfile << buf2 << ",";
        }
        actfile << "\n";
    }
...
int main (int argc, char *argv[])
{
    ...
    for(int i = 0; i < POPSIZE; i++){
        ...
        for(int j = 0; j < TWDCONTCOUNT*TWDINPUTS; j++){
            ...
            if(weightnum == 18){
		double dummydub = mutnd(mutrd);
                initgeno.push_back(dummydub);
            }else{
                initgeno.push_back((double)((rand() % 11)-5)/100);	
            }
  	     //printf("initgeno %i %i %f \n", i, j, initgeno.at(j));
        }
    ...
    if(PLAYBACK){
	wingeno = loadedwin;
	printf("loaded winner geno\n");
        for(int i = 0; i < wingeno.size(); i++){
		printf("%f\n",wingeno.at(i));	
	}
    }
