...
#define PRESTART 1	//do you want to load in a walker before running the ga for bootstrapping
#define PLAYBACK 0	//load in a walker for visualisation or run ga
...
struct Walker {
    ...
    void init(){
		...
		//making sure bodgeno scale value is within range
        if(bodgeno.at(0) <= 1){
            bodgeno.at(0) = 1;
        }
        if(bodgeno.at(0) >= 2){
		...
		//bod mutation rate also must be over 0
        if(bodgeno.at(1) <= 0){
            bodgeno.at(1) = 0.01;
        }
		...
		r_th_ang = 45*DEG2RAD;
        dBodySetPosition (body[2], -((UPPERLENGTH*bodgeno.at(0))/2)*sin(r_th_ang), -(HIPLENGTH/2),startz-(HIPRADIUS/2)-(((UPPERLENGTH*bodgeno.at(0))/2)*cos(r_th_ang)));
        //set thigh rotation
        dMatrix3 rot_ptr;
		...
		l_th_ang = 15*DEG2RAD;
        dBodySetPosition (body[3], -((UPPERLENGTH*bodgeno.at(0))/2)*sin(l_th_ang), (HIPLENGTH/2),startz-(HIPRADIUS/2)-(((UPPERLENGTH*bodgeno.at(0))/2)*cos(l_th_ang)));
        //set thigh rotation
        dMatrix3 rot_ptr2;
        ...
        dJointSetHingeAnchor (joint[1],d1[0] + ((UPPERLENGTH*bodgeno.at(0))/2)*sin(r_th_ang),d1[1], d1[2] +((UPPERLENGTH*bodgeno.at(0))/2)*cos(r_th_ang));
        ...
        dJointSetHingeAnchor (joint[2],d2[0] + ((UPPERLENGTH*bodgeno.at(0))/2)*sin(l_th_ang),d2[1], d2[2] +((UPPERLENGTH*bodgeno.at(0))/2)*cos(l_th_ang));
        ...
        dReal kneepos_ptr[3];
		kneepos_ptr[0] = hippos_ptr[0] - (UPPERLENGTH*bodgeno.at(0))*sin(r_th_ang);
		kneepos_ptr[1] = hippos_ptr[1]+0.0140;
		kneepos_ptr[2] = hippos_ptr[2]-(UPPERLENGTH*bodgeno.at(0))*cos(r_th_ang);
        ...
        r_sh_ang = -45*DEG2RAD;
        dBodySetPosition (body[4], kneepos_ptr[0] - ((LOWERLENGTH*bodgeno.at(0))/2)*sin(r_sh_ang) + 0.0011*cos(r_sh_ang), kneepos_ptr[1] +0.0140, kneepos_ptr[2]- ((LOWERLENGTH*bodgeno.at(0))/2)*cos(r_sh_ang) + 0.0011*sin(r_sh_ang));
        dMatrix3 rot_ptr3;
		...
		kneepos_ptr2[0] = hippos_ptr2[0] - (UPPERLENGTH*bodgeno.at(0))*sin(l_th_ang);
		kneepos_ptr2[1] = hippos_ptr2[1]-0.0140;
		kneepos_ptr2[2] = hippos_ptr2[2]-(UPPERLENGTH*bodgeno.at(0))*cos(l_th_ang);
        ...
        l_sh_ang = -60*DEG2RAD;
        dBodySetPosition (body[5], kneepos_ptr2[0] - ((LOWERLENGTH*bodgeno.at(0))/2)*sin(l_sh_ang) + 0.0011*cos(l_sh_ang), kneepos_ptr2[1]-0.0140, kneepos_ptr2[2]- ((LOWERLENGTH*bodgeno.at(0))/2)*cos(l_sh_ang) + 0.0011*sin(l_sh_ang));
        dMatrix3 rot_ptr4;
        ...
        //right foot
        dReal ank_ptr[3];
		ank_ptr[0] = kneepos_ptr[0]-0.0011-(LOWERLENGTH*bodgeno.at(0))*sin(r_sh_ang);
		ank_ptr[1] = kneepos_ptr[1];
		ank_ptr[2] = kneepos_ptr[2]-(LOWERLENGTH*bodgeno.at(0))*cos(r_sh_ang);
        ...
        //left foot
        dReal ank_ptr2[3];
		ank_ptr2[0] = kneepos_ptr2[0]-0.0011-(LOWERLENGTH*bodgeno.at(0))*sin(l_sh_ang);
		ank_ptr2[1] = kneepos_ptr2[1];
		ank_ptr2[2] = kneepos_ptr2[2]-(LOWERLENGTH*bodgeno.at(0))*cos(l_sh_ang);
        ...
        dJointSetHingeAnchor(joint[5],ank_ptr[0],ank_ptr[1],ank_ptr[2]);
        dJointSetHingeAxis (joint[5],0,1,0);
        dJointSetHingeParam (joint[5],dParamHiStop,115*DEG2RAD);
        dJointSetHingeParam (joint[5],dParamLoStop,-25*DEG2RAD);

        joint[6] = dJointCreateHinge(world,0);
        dJointAttach (joint[6],body[5],body[7]);
        dJointSetHingeAnchor(joint[6],ank_ptr2[0],ank_ptr2[1],ank_ptr2[2]);
        dJointSetHingeAxis (joint[6],0,1,0);
        dJointSetHingeParam (joint[6],dParamHiStop,120*DEG2RAD);
        dJointSetHingeParam (joint[6],dParamLoStop,-20*DEG2RAD);
		...
		dReal startratio = 0.1/0.33;
        dBodySetPosition (exbody[0],0,0,SPHERERAD + (startratio*startz));
        ...
        dBodySetPosition (exbody[1],SPHERE2POS,0,SPHERERAD + (startratio*startz) + 0.05);
	...
	//EXTRA JOINTS
        ...
        dJointSetHingeAnchor (exjoint[1],SPHERE2POS,0,SPHERERAD + (startratio*startz) + 0.05);
...
void geneticAlg(){
        ...
        for(int k = 0; k < GENS; k++){
            ...
            for(int i = ELITNUM; i < POPSIZE; i++){
				//replacement array for new genotypes
                ...
                if(propselect > 24){ //crossover
					...
					//choose genes for body replacement from random parents
...
void writeFilesSetSeed(){
	...
	//bod genotype file, written in hexadecimal to avoid rounding inconsistencies on reload
...
int main (int argc, char *argv[])
{
    ...
    if(PRESTART){
    	...
    	for(int i = 0; i < POPSIZE; i++){
			...
			//initialising body genotype				
    	    std::vector<double> initbod;
    	    //push back a normal input and a mutation rate
