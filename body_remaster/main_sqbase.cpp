...
struct Walker {
    ...
    void init(){
        ...
        if(THDINIT){
            //leg raise
            dBodySetPosition (body[3], ((UPPERLENGTH/2)*sin(init3dang)), (HIPLENGTH/2),startz-0.025-((UPPERLENGTH/2)*cos(init3dang)));
            //set thigh rotation
            dMatrix3 rot_ptr;
        ...
        if(THDINIT){
            ...
            dJointSetHingeAnchor (joint[2],d2[0] - (UPPERLENGTH/2)*sin(init3dang),d2[1], d2[2] + (UPPERLENGTH/2)*cos(init3dang));
        }else{
        ...
        if(THDINIT){
            //leg raise
            dBodySetPosition (body[5], hippos_ptr[0] + (UPPERLENGTH+(LOWERLENGTH/2))*sin(init3dang) + 0.0011*cos(init3dang),hippos_ptr[1]-0.0140,hippos_ptr[2]-(UPPERLENGTH+(LOWERLENGTH/2))*cos(init3dang) + 0.0011*sin(init3dang));
        ...
        if(THDINIT){
            //leg raise
            dJointSetHingeAnchor(joint[4],hippos_ptr[0] + (UPPERLENGTH)*sin(init3dang) ,hippos_ptr[1]-0.0140,hippos_ptr[2]-(UPPERLENGTH)*cos(init3dang));
        }else{
        ...
        if(THDINIT){
            //leg raise
            dBodySetPosition (body[7], kneepos_ptr[0]+0.053*cos(init3dang) +(LOWERLENGTH)*sin(init3dang),kneepos_ptr[1],kneepos_ptr[2]+ 0.053*sin(init3dang) -(LOWERLENGTH)*cos(init3dang));
            //set angle rotation
            dMatrix3 rot_ptr3;
        ...
        if(THDINIT){
            //leg raise
            dJointSetHingeAnchor(joint[6],kneepos_ptr[0]+0.0011+(LOWERLENGTH)*sin(init3dang),kneepos_ptr[1],kneepos_ptr[2]-(LOWERLENGTH)*cos(init3dang));
        }else{
