...
#define PRESTART 0 //do you want to load in a walker before running the ga for bootstrapping
...
struct Walker {
    ...
    void init(){
        ...
        if(LEGRAISE){
            //leg raise
            dBodySetPosition (body[3], (((UPPERLENGTH*bodgeno.at(0))/2)*sin(initang)), (HIPLENGTH/2),startz-0.025-(((UPPERLENGTH*bodgeno.at(0))/2)*cos(initang)));
            //set thigh rotation
            dMatrix3 rot_ptr;
        ...
        if(LEGRAISE){
            ...
            dJointSetHingeAnchor (joint[2],d2[0] - ((UPPERLENGTH*bodgeno.at(0))/2)*sin(initang),d2[1], d2[2] + ((UPPERLENGTH*bodgeno.at(0))/2)*cos(initang));
        }else{
        ...
        if(LEGRAISE){
            //leg raise
            dBodySetPosition (body[5], hippos_ptr[0] + ((UPPERLENGTH*bodgeno.at(0))+((LOWERLENGTH*bodgeno.at(0))/2))*sin(initang) + 0.0011*cos(initang),hippos_ptr[1]-0.0140,hippos_ptr[2]-((UPPERLENGTH*bodgeno.at(0))+((LOWERLENGTH*bodgeno.at(0))/2))*cos(initang) + 0.0011*sin(initang));
        ...
        if(LEGRAISE){
            //leg raise
            dJointSetHingeAnchor(joint[4],hippos_ptr[0] + (UPPERLENGTH*bodgeno.at(0))*sin(initang) ,hippos_ptr[1]-0.0140,hippos_ptr[2]-(UPPERLENGTH*bodgeno.at(0))*cos(initang));
        }else{
        ...
        if(LEGRAISE){
            //leg raise
            dBodySetPosition (body[7], kneepos_ptr[0]+0.053*cos(initang) +(LOWERLENGTH*bodgeno.at(0))*sin(initang),kneepos_ptr[1],kneepos_ptr[2]+ 0.053*sin(initang) -(LOWERLENGTH*bodgeno.at(0))*cos(initang));
            //set angle rotation
            dMatrix3 rot_ptr3;
        ...
        if(LEGRAISE){
            //leg raise
            dJointSetHingeAnchor(joint[6],kneepos_ptr[0]+0.0011+(LOWERLENGTH*bodgeno.at(0))*sin(initang),kneepos_ptr[1],kneepos_ptr[2]-(LOWERLENGTH*bodgeno.at(0))*cos(initang));
        }else{
